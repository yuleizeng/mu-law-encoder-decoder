...
% 设置量化参数
quantization_values = [10, 100, 200, 300, 400]; % 选择的量化参数
...
% 绘制原始音频
subplot(length(quantization_values)+1, 3, 1);
plot(y1);
title('Original Audio');

% 循环进行编码和解码，比较信噪比
for i = 1:length(quantization_values)
    % 编码
    encoded_signal = encode_signal(y, quantization_values(i),y_scale);
    
    % 解码
    decoded_signal = decode_signal(encoded_signal, quantization_values(i),y_scale);
    
    %解码后缩小
    decoded_signal=decoded_signal*max_y;  
    
    % 绘制编码后的音频
    subplot(length(quantization_values)+1, 2, i*2+1);
    plot(encoded_signal);
    title(['Encoded Audio (Parameter = ', num2str(quantization_values(i)), ')']);
    
    % 指定文件名
    filename = ['mu-law (Parameter =' num2str(quantization_values(i)) ').wav' ];
     
    % 保存WAV文件
    audiowrite(filename, decoded_signal, fs);

    % 绘制解码后的音频()
    subplot(length(quantization_values)+1, 2, i*2+2);
    plot(decoded_signal);
    title(['Decoded Audio (Parameter = ', num2str(quantization_values(i)), ')']);
...
disp(snr_values);
% 调整子图布局
sgtitle('Mu-law Encoding and Decoding');
